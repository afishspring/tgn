...
def get_data_node_classification(dataset_name, use_validation=False):
    ...
    val_mask = np.logical_and(
        timestamps <= test_time, timestamps > val_time) if use_validation else test_mask
...
def get_data(dataset_name, different_new_nodes_between_val_and_test=False, randomize_features=False):
    ...
    val_time, test_time = list(np.quantile(graph_df.ts, [0.3, 0.7]))
    ...
    new_test_node_set = set(random.sample(
        list(test_node_set), int(0.1 * n_total_unique_nodes)))
