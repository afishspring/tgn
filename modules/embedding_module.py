...
class GraphEmbedding(EmbeddingModule):
    ...
    def compute_embedding(self, memory, source_nodes, timestamps, n_layers, n_neighbors=20, time_diffs=None,
        ...
        source_nodes_torch = torch.from_numpy(
            source_nodes).long().to(self.device)
        timestamps_torch = torch.unsqueeze(torch.from_numpy(
            timestamps).float().to(self.device), dim=1)
        ...
        if n_layers == 0:
            ...
            neighbors_torch = torch.from_numpy(
                neighbors).long().to(self.device)
            ...
            edge_deltas_torch = torch.from_numpy(
                edge_deltas).float().to(self.device)

            neighbors = neighbors.flatten()
            neighbor_embeddings = self.compute_embedding(memory,
                                                         neighbors,
                                                         np.repeat(
                                                             timestamps, n_neighbors),
                                                         n_layers=n_layers - 1,
                                                         n_neighbors=n_neighbors)
...
class GraphSumEmbedding(GraphEmbedding):
    ...
    def aggregate(self, n_layer, source_node_features, source_nodes_time_embedding,
        ...
        neighbors_sum = torch.nn.functional.relu(
            torch.sum(neighbor_embeddings, dim=1))
...
def get_embedding_module(module_type, node_features, edge_features, memory, neighbor_finder,
    ...
    if module_type == "graph_attention":
        ...
        raise ValueError(
            "Embedding Module {} not supported".format(module_type))
